...
alias glt="git log --color --graph --pretty=format:'%C(red)%h%C(reset) - %<(50,trunc)%s %C(green)%<(15,trunc)%cr %C(bold blue)%<(15,trunc)%an %C(reset)%C(yellow)%d' --abbrev-commit | less -R"
alias gb='git branch'
